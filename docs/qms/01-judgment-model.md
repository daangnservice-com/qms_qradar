...
1. **검토 필요 / 불필요**
   - 필요: AI가 검토필요라고 한 것이 맞다. (`+` 미검출도 필요)
   - 불필요: AI 과검출
2. **최종 Cold / Hot** (검토 필요인 경우만)
   - Cold: 감안 불가 (부적합)
   - Hot: 감안 (적합)
...
- `humanFinalLabel` = 최종 Cold 1개 이상이면 `cold`, 아니면 `hot` (매트릭스에 넣지 않음)

## Gate (AI 정확도)

- 목표: 항목별 AI–사람 **검토필요 일치율 90%**, 연속 통과 시 운영 전환
- 양성 클래스 = 검토 필요. Precision/Recall은 이 축 기준.
- 수기 최종 Cold/Hot은 이후 AI가 최종 판정까지 도전할 때의 골드. 지금은 Gate에 쓰지 않는다.
...
- **9월~**: AI = 검토필요. 수기는 검토필요 + 최종 Cold/Hot. 신규 입력부터 신정의.
